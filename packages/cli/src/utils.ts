...
export async function createSkeletons(projectDir: string, type = 'skeleton') {
  ...
  try {
    files = await new Promise((resolve, reject) => {
      return readdir(join(getTemplatesDir(), type), (error, files) =>
  ...
  try {
    await Promise.all(
      files.map(async (file) =>
        copy(join(getTemplatesDir(), type, file), join(projectDir, file))
      )
    );
  } catch (error) {
...
export async function writeJsonFile(
  ...
  try {
    await writeJson(join(projectDir, filename), schema, {
      spaces: 2,
      EOL: '\r\n',
    });
  } catch (error) {
